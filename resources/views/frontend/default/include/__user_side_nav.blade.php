            ...
            <div class="sidebar-nav">
                <nav class="user-nav">
                    <ul>
                        ...
                        @if ($navigation->type == 'referral' && setting('sign_up_referral', 'permission') &&
                        auth()->user()->referral_status)
                        @include('frontend::include.__menu-item',['navigation' => $navigation])
                        @elseif ($navigation->type == 'deposit' && setting('user_deposit', 'permission') &&
                        auth()->user()->deposit_status)
                        @include('frontend::include.__menu-item',['navigation' => $navigation])
                        @elseif ($navigation->type == 'withdraw' && setting('user_withdraw', 'permission') &&
                        auth()->user()->withdraw_status)
                        @include('frontend::include.__menu-item',['navigation' => $navigation])
                        @elseif ($navigation->type == 'fund_transfer' && setting('transfer_status', 'permission') &&
                        auth()->user()->transfer_status)
                        @include('frontend::include.__menu-item',['navigation' => $navigation])
                        @elseif ($navigation->type == 'dashboard' || $navigation->type == 'subscriptions' ||
                        $navigation->type == 'my_ads' || $navigation->type == 'support' || $navigation->type ==
                        'transactions' || $navigation->type == 'settings' || $navigation->type == 'logout')
                        @include('frontend::include.__menu-item',['navigation' => $navigation])
                        @endif
                        @endforeach
                    </ul>
