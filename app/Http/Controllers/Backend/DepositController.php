...
class DepositController extends Controller
{
    ...
    public function pending(Request $request)
    {
        ...
        $deposits = Transaction::with('user')
                            ...
                            ->whereIn('type', [TxnType::ManualDeposit->value,TxnType::PlanPurchased->value])
                            ->search($search)
    ...
    public function history(Request $request)
    {
        ...
        $deposits = Transaction::with('user')
                    ->whereIn('type',[TxnType::ManualDeposit->value,TxnType::Deposit->value,TxnType::PlanPurchased->value])
                    ->search($search)
                    ->when(in_array(request('sort_field'),['created_at','amount','charge','method','status','tnx']),function($query){
                        $query->orderBy(request('sort_field'),request('sort_dir'));
                    })
    ...
    public function actionNow(Request $request)
    {
        ...
        if (isset($input['approve'])) {
            ...
            if ($transaction->type == TxnType::PlanPurchased) {
                $plan =  SubscriptionPlan::find($transaction->plan_id);
                $this->executePlanPurchaseProcess($transaction->user,$plan);

                //level referral
                if (setting('site_referral', 'global') == 'level' && setting('subscription_plan_level')) {
                    $level = LevelReferral::where('type', 'subscription_plan')->max('the_order') + 1;
